...
- **Choosing which browser window Pi works in — once, machine-wide.** `/chrome window` — and
  "Choose window…" in the dashboard — lists the open windows and lets Pi's tab go into one of them
  instead; the window is identified by the title of the tab you can actually see in it. The pick is
  saved as `preferredWindow` in `~/.pi/agent/pi-chrome.json` and every new session inherits it, so the
  choice is made once instead of per session. The tab is created inactive and the window is never
  focused, and since that window is yours, cleanup closes only Pi's tab in it and never the window
  itself. pi-chrome never creates a window of its own: with no saved choice and no assignment, an
